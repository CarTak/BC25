...
-- Entidades: Vendedor, Produto, Enddereço
...
CREATE TABLE produto(
    ...
    desconto DECIMAL(4,2) NOT NULL DEFAULT(0), -- Caso não forneça valor p/ desconto ele irá utilizar 0
    descontoDate DATE,
...
-- TABELA ENDEREÇO: id, cidade, uf, rua, bairro, numero, completo, idVendedor
CREATE TABLE endereco(
idEndereco INTEGER PRIMARY KEY AUTO_INCREMENT,
cidade VARCHAR(30) NOT NULL, -- tamanho variável
uf CHAR(2) NOT NULL, -- tamanho fixo
rua VARCHAR(30) NOT NULL,
bairro VARCHAR(30) NOT NULL,
numero VARCHAR(15) NOT NULL,
complemento VARCHAR(30) NOT NULL,
idVendedor INTEGER UNIQUE NOT NULL, -- Com o UNIQUE garantimos o relacionamento 1:1 (O vendedor não poderá ter mais de um endereço)
FOREIGN KEY(idVendedor) REFERENCES vendedor(idVendedor)
);
...
SELECT NOW() AS agora;
SELECT 1 + 1 AS resultado; -- ALIAS (resultado, agora) = apelido
SELECT nome FROM vendedor; -- Podemos escolher quais colunas ver na tabela
SELECT nome, sobrenome FROM vendedor;
SELECT dataNasc AS aniversario FROM vendedor; -- aniversario só aparecera na listagem não há alteração
SELECT nome AS nomeProduto, preco FROM produto;

-- SELEÇÃO: É filtrar os registros da tabela
SELECT idvendedor, nome, sobrenome FROM vendedor WHERE idVendedor = 1; -- Lista vendedor com id = 1
SELECT * FROM vendedor WHERE idVendedor != 1; -- Listar todos menos o vendedor com id = 1
SELECT * FROM vendedor WHERE idVendedor = 1 or idVendedor = 2; -- lista todas as colunas, onde o vendedor é 1 ou 2
SELECT * FROM vendedor WHERE idVendedor != 1 or idVendedor != 2;  -- lista todas as linhas, onde o vendedor é diferente de 1 ou 2
SELECT * FROM vendedor WHERE email = "jr@email.com"; -- Resultado vazio, sem registros
SELECT * FROM produto WHERE preco > 300;
SELECT * FROM produto WHERE preco >= 300;
SELECT * FROM produto WHERE preco < 100;
SELECT * FROM produto WHERE preco <= 100;
SELECT * FROM produto WHERE preco >= 100 AND preco <= 500; -- Listando produtos da faixa de preço 100 e 500
SELECT * FROM produto WHERE desconto > 0.0 AND descontoDate < CURRENT_DATE(); -- Lista produtos com data expirada
SELECT CURRENT_DATE();
SELECT * FROM produto WHERE quantidade < 20; -- Lista produtos com estoque abaixo de 20
SELECT * FROM produto WHERE quantidade = 0; -- Lista produtos sem estoque
SELECT * FROM vendedor WHERE YEAR(dataNasc) < 2000;
SELECT * FROM produto WHERE quantidade >= 20 AND quantidade <= 100;
...
SELECT * FROM produto WHERE descontoDate IS NULL; -- Lista produtos em que a data é nula
SELECT * FROM produto WHERE descontoDate IS NOT NULL; -- Lista produtos em que a data não é nula
...
SELECT * FROM vendedor WHERE email LIKE "%gnail.com"; -- Busca o @gmail.com no final
SELECT * FROM vendedor WHERE email LIKE "%.org";-- Busca o .org no final
SELECT * FROM vendedor WHERE dataNasc LIKE "%-10-%";
SELECT COUNT(*) FROM vendedor WHERE dataNasc LIKE "%-10-%";
...
SELECT * FROM vendedor ORDER BY nome, sobrenome; -- Quando nomes iguais o sobrenome desempata
...
SELECT * 
FROM produto 
WHERE desconto > 0.0 AND descontoDate > CURRENT_DATE() 
ORDER BY preco DESC 
LIMIT 50; -- podemos quebrar linha para organizar melhor
-- LIMIT 50 => os PRIMEIROS RESULTADOS DA QUERY

SELECT idVendedor FROM endereco WHERE uf = "CE"; -- Listar o id dos vendedores do CE
SELECT idVendedor FROM endereco WHERE uf IN("SP", "RJ", "PI");
SELECT * FROM vendedor WHERE YEAR(dataNasc) > 2000; -- Lista os vendedores que nasceram d´pois de 2000
SELECT * FROM vendedor WHERE MONTH(dataNasc) = 10; -- Lista os vendedores que nasceram no mes 10

SELECT * FROM vendedor WHERE MONTH(dataNasc) = MONTH(CURRENT_DATE()); -- Lista os vendedores que nasceram no mes corrente

-- 1) Listar os produtos com desconto maior que 50;
SELECT * FROM produto WHERE desconto >= 0.5;

-- 2) Listar os produtos que estão abaixo do estoque de 5;
SELECT * FROM produto WHERE quantidade < 5;

-- 3) Listar os produtos que estavam em promoção ano passado;
SELECT * FROM produto WHERE YEAR(descontoDate) = 2021;

-- 4) Listar os vendedores nascidos entre 1990 e 2001;
SELECT * FROM vendedor WHERE YEAR(dataNasc) BETWEEN 1990 AND 2001;

-- 5) Listar o ID dos vendedores que moram na cidade de Campinas
SELECT * FROM endereco WHERE cidade = "Campinas";
...
INNER JOIN endereco 
ON vendedor.idVendedor = endereco.idVendedor
WHERE uf = "CE"; -- pode ser uf IN("SP","CE","PI")
...
SELECT produto.nome AS produtoNome , preco, desconto, descontoDate
FROM vendedor
	INNER JOIN produto
    ON vendedor.idVendedor = produto.idVendedor
    INNER JOIN endereco
    ON vendedor.idVendedor = endereco.idVendedor
WHERE desconto > 0.0 AND descontoDate > CURRENT_DATE() AND uf = "CE"; -- uf IN("SP","CE","PI")
...
-- ADICONAR NOVAS COLUNAS SEM DROPAR

ALTER TABLE produto
ADD COLUMN precoMinimo DECIMAL(10, 2) NOT NULL DEFAULT(1); -- Adiciono uma nova coluna, as linhas atuais recebem o valor 1 por padrão

-- RENOMEAR COLUNA

ALTER TABLE produto
CHANGE precoMinimo preco_Minimo DECIMAL(8, 2) NOT NULL DEFAULT(2); -- Você pode Alterar e Renomear totalmente a coluna

-- EXCLUI COLUNA

ALTER TABLE produto
DROP COLUMN preco_Minimo; -- Remove a coluna e dados dela completamente

-- Funções de agregação 

SELECT MAX(desconto) FROM produto; -- Maior desconto da tabela
SELECT MIN(desconto) FROM produto; -- Menor desconto da tabela
SELECT MAX(preco) FROM produto; -- Maior preco da tabela
SELECT MIN(preco) FROM produto; -- Menor preco da tabela

SELECT AVG(preco) FROM produto; -- Média de preços dos produtos

SELECT SUM(preco) FROM produto; -- SOma dos precos do produto
...
SELECT *
FROM produto
WHERE preco > (SELECT AVG(preco) FROM produto); -- SUBQUERY que retorna produtos com preço acima da média

SELECT *
FROM produto
WHERE desconto < (SELECT AVG(desconto) FROM produto); -- SUBQUERY que retorna os produtos com desconto abaixo da média

SELECT AVG(quantidade) FROM produto; -- Média de quantidade dos produtos
SELECT MAX(quantidade) FROM produto; -- Maior quantidade em estoque
